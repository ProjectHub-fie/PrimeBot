/*
 * PrimeBot Dashboard — inline SVG icon system.
 *
 * Single shared catalog of 1.75px-stroke, currentColor SVG path data used
 * across the dashboard (server-rendered markup + client JS). Keeping the
 * catalog in one place lets both the server-side renderers (dashboard/render)
 * and the client scripts reference the same icons by name without duplicating
 * path strings.
 *
 * svgIcon(name, cls?) returns an <svg class="ico …">…</svg> string. Unknown
 * names render an empty (zero-content) svg so a missing icon never breaks
 * layout — only the glyph is absent.
 *
 * Ported from PR #16's SPA app.js (MinexInd) onto the current MPA. Emoji are
 * kept only for user-entered Discord emoji placeholders (reaction / ticket /
 * badge pickers), never as UI affordances.
 */
...
const ICONS = {
  ...
  role: '<circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/>',
  ...
  grip: '<circle cx="9" cy="6" r="1.6"/><circle cx="9" cy="12" r="1.6"/><circle cx="9" cy="18" r="1.6"/><circle cx="15" cy="6" r="1.6"/><circle cx="15" cy="12" r="1.6"/><circle cx="15" cy="18" r="1.6"/>',
  ...
  flask: '<path d="M10 2v7.31a3 3 0 0 1-.64 1.9L3.6 18.5A2 2 0 0 0 5.2 21.7h13.6a2 2 0 0 0 1.6-3.2l-5.76-7.29A3 3 0 0 1 14 9.3V2"/><path d="M8.5 2h7"/><path d="M7 16h10"/>',
  ...
  inboxArchive: '<polyline points="22 12 16 12 14 15 10 15 8 12 2 12"/><path d="M5.45 5.11 2 12v6a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2v-6l-3.45-6.89A2 2 0 0 0 16.76 4H7.24a2 2 0 0 0-1.79 1.11z"/><line x1="12" y1="12" x2="12" y2="16"/>',
  ...
  octagonX: '<polygon points="7.86 2 16.14 2 22 7.86 22 16.14 16.14 22 7.86 22 2 16.14 2 7.86 7.86 2"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/>',
...
// Expose globally (no module bundler on the dashboard client).
if (typeof window !== 'undefined') {
  window.ICONS = ICONS;
  window.svgIcon = svgIcon;
}
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { ICONS, svgIcon };
}
